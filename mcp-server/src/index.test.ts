...
vi.mock('./handlers.js', async (importOriginal) => {
  ...
  return {
    ...
    securityScanAll: vi.fn().mockResolvedValue({ trivy: null, sonarqube: null, dependencyTrack: null }),
...
describe('toolDefinitions', () => {
  ...
  it('should have 24 tools total', () => {
    expect(toolDefinitions.length).toBe(24);
  });
...
describe('handleCallTool', () => {
  ...
  describe('SonarQube tools', () => {
    ...
    it('should handle sonar_get_issues with types', async () => {
      const result = await handleCallTool('sonar_get_issues', { projectKey: 'my-project', types: 'BUG' });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });

    it('should handle sonar_get_security_hotspots', async () => {
      const result = await handleCallTool('sonar_get_security_hotspots', { projectKey: 'my-project' });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });
  ...
  describe('Dependency-Track tools', () => {
    ...
    it('should handle dtrack_get_vulnerabilities', async () => {
      const result = await handleCallTool('dtrack_get_vulnerabilities', { projectUuid: 'uuid-123' });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });
  ...
  describe('Gitea tools', () => {
    ...
    it('should handle gitea_get_commits with limit', async () => {
      const result = await handleCallTool('gitea_get_commits', { owner: 'admin', repo: 'test', limit: 5 });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });
  ...
  describe('Drone tools', () => {
    ...
    it('should handle drone_get_build', async () => {
      const result = await handleCallTool('drone_get_build', { owner: 'admin', repo: 'test', build: 1 });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });

    it('should handle drone_get_build_logs', async () => {
      const result = await handleCallTool('drone_get_build_logs', { owner: 'admin', repo: 'test', build: 1 });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });
    ...
    it('should handle drone_trigger_build with branch', async () => {
      const result = await handleCallTool('drone_trigger_build', { owner: 'admin', repo: 'test', branch: 'develop' });
      expect(result.content).toBeDefined();
      expect(result.isError).toBeUndefined();
    });
  });
